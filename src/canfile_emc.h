...
//64
	...
	unsigned char unknown16[10];				// 10 bytes
...
//138
	...
	unsigned char unknown20[8];					// 8 bytes
	unsigned char min_temp_bt[2];    			// 2 bytes [seconds]
//150
	unsigned char unknown21[22];				// 22 bytes
//172
...
//178
	unsigned char unknown22[1];					// 1 byte
	unsigned char profile_period;				// Second between profile samples
	unsigned char unknown23[36];				// 38 bytes
//216
...
// 0x13e end of inter-dive bytes (maybe pointer to last inter-dive byte)
// 0x142 end of last dive (4 bytes LE) (inter-dive bytes follow)
